        ...
        case 'list':
            ...
            if (Array.isArray(value)) {
                for (k in value) {
                    ...
                    if (!PAYLOAD_PROPERTIES[prop].pr_allowCommas
                        && value[k].toString().indexOf(',') !== -1
                        && errors.bad_values.indexOf(prop) === -1) {
        ...
        if (data.hasOwnProperty('manifest')) {
            ...
            if (manifest.hasOwnProperty('requirements')) {
                ...
                if (req.hasOwnProperty('bootrom')) {
                    ...
                    // If the image requirements don't specify a bootrom, then
                    // we allow it to be chosen by the VM manifest.
                    // But we don't allow a path to a bootrom at provision time
                    // because there's no way to get a custom rom into the
                    // filesystem until after the zone is created, and a path
                    // to a non-existent file is guaranteed to fail to boot.
                    // TODO: Allow paths at provision time if autoboot is set
                    // to false?
                    if (VM.BOOTROMS.indexOf(image.bootrom) === -1) {
                        e = new Error('invalid bootrom value for image '
                            + image.uuid + ' passed: ' + image.bootrom
                            + ' should be one of: ' + VM.BOOTROMS.join(', '));
                        e.whatFailed = 'EBADBOOTROM';
                        log.error(e);
                        callback(e);
                        return;
    ...
    getZpools(log, function (err, zpools) {
        ...
        if (BRAND_OPTIONS[brand].hasOwnProperty('allowed_properties')
            ...
            function validateDiskSource(prop_prefix, disk) {
                ...
                if (disk.hasOwnProperty('image_uuid')) {
                    // With image_uuid, image_size is required, block_size is
                    // not allowed, and size is not allowed unless
                    // flexible_disk_size is also set.
                    ...
                    if (disk.hasOwnProperty('size')
                        && !payload.hasOwnProperty('flexible_disk_size')) {
        ...
        if (BRAND_OPTIONS[brand].hasOwnProperty('allowed_properties')
            ...
            function validatePCIDevice(prop_prefix, dev) {
                ...
                if (dev.hasOwnProperty('pptdev')
                    && dev.pptdev.indexOf('/dev/ppt') !== 0) {
                    errors.bad_values.push(prop_prefix + '.pptdev');
                }

                if (dev.hasOwnProperty('rom')) {
                    if (dev.rom.indexOf('/') !== 0) {
                        errors.bad_values.push(prop_prefix + '.rom');
                    }
                } else if (dev.hasOwnProperty('rom_exec')) {
                    errors.missing_properties.push(prop_prefix + '.rom');
                }
            }
...
function zonecfgQuoteValue(value)
{
...
function buildDiskZonecfg(vmobj, payload)
{
    ...
    for (disk in remove) {
        disk = remove[disk];
        zcfg = zcfg + 'remove -F device match='
            + zonecfgQuoteValue(disk) + '\n';
    }
    ...
    for (disk in add) {
        ...
        zcfg = zcfg + 'add device\n'
            + 'set match=' + zonecfgQuoteValue(disk.path) + '\n'
...
function buildPCIDeviceZonecfg(vmobj, payload)
{
    ...
    for (device in add) {
        ...
        zcfg = zcfg + 'add device\n'
            + 'set match=' + zonecfgQuoteValue(device.path) + '\n'
            + 'add property (name=model, value="passthru")\n'
            + 'add property (name=pci-slot, value="' + device.pci_slot + '")\n';

        if (device.hasOwnProperty('rom')) {
            zcfg = zcfg
                + 'add property (name=rom, value="' + device.rom + '")\n';
        }

        if (device.hasOwnProperty('pptdev')) {
            zcfg = zcfg + 'add property (name=pptdev, value="'
                + device.pptdev + '")\n';
        }

        if (device.hasOwnProperty('rom_exec')) {
            zcfg = zcfg + 'add property (name=rom-exec, value="'
                + device.rom_exec + '")\n';
        }

        if (device.hasOwnProperty('msi')) {
            zcfg = zcfg + 'add property (name=msi, value="'
                + device.msi + '")\n';
        }
...
function buildZonecfgUpdate(vmobj, payload, log)
{
    ...
    if (brand === 'bhyve') {
        ...
        setAttr('hostbridge', 'bhyve_hostbridge');
        setAttr('tpm', 'bhyve_tpm');
        setAttr('virtio1', 'virtio1');
...
/*
 * This function checks for ZFS holds on any snapshots of the specified dataset.
 * It will fail if any holds are found, preventing destructive operations that
 * would fail due to held snapshots.
 *
 * Parameters:
 *   dataset - the ZFS dataset to check (e.g., 'zones/uuid/disk0')
 *   log     - logger object
 *   callback - function(err) called with error if holds found, null if clear
 */
function checkDiskHolds(dataset, log, callback)
{
    var tracers_obj;

    if (process.env.EXPERIMENTAL_VMJS_TRACING) {
        tracers_obj = traceUntilCallback('check-disk-holds', log, callback);
        callback = tracers_obj.callback;
        log = tracers_obj.log;
    }

    log.debug('checking for holds on dataset %s', dataset);

    // First, list all snapshots for this dataset
    zfs(['list', '-r', '-t', 'snapshot', '-Ho', 'name', dataset], log,
        function _listSnapshots(list_err, list_out) {
            var held_snapshots = [];
            var snapshot_lines;

            if (list_err) {
                // If dataset doesn't exist, that's fine - no snapshots to check
                if (list_out && list_out.stderr
                    && list_out.stderr.match(/dataset does not exist/)) {
                    log.debug('dataset %s does not exist, no holds to check',
                        dataset);
                    callback();
                    return;
                }
            ...
            snapshot_lines = list_out.stdout.trim().split('\n');
            if (snapshot_lines.length === 1 && snapshot_lines[0] === '') {
                // No snapshots found
                log.debug('no snapshots found on %s', dataset);
                callback();
                return;
            }

            log.debug('found %d snapshots to check for holds',
                snapshot_lines.length);

            // Check each snapshot for holds
            async.eachSeries(snapshot_lines, function (snapshot, cb) {
                if (!snapshot || snapshot.trim() === '') {
                    cb();
                    return;
                }

                zfs(['holds', snapshot.trim()], log,
                    function _checkSnapshotHolds(holds_err, holds_out) {
                        var holds = [];
                        var lines;
                        ...
                        if (holds.length > 0) {
                            ...
                            log.debug('snapshot %s has %d holds: %s',
                                snapshot.trim(), holds.length,
                                holds.join(', '));
                        }
                ...
                // If we found any held snapshots, fail with detailed error
                if (held_snapshots.length > 0) {
                    var error_lines = [
                        'Cannot reprovision: disk0 has snapshots with holds '
                        + 'that must be released first:'
                    ];

                    held_snapshots.forEach(function (snap_info) {
                        error_lines.push(sprintf('  - %s (holds: %s)',
                            snap_info.snapshot, snap_info.holds.join(', ')));
                    });

                    callback(new Error(error_lines.join('\n')));
                    return;
                }

                log.debug('no holds found on any snapshots of %s', dataset);
                callback();
            });
        });
}
...
function installZone(payload, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // When receiving zoneinit is never run, instead detach/attach the
            // zone to ensure the cores dataset gets created properly by the
            // brand scripts.
            if (receiving) {
                vasync.pipeline(
                    {funcs: [
                        function (_, cb2) {
                            zoneadm(['-u', payload.uuid, 'detach'], log, cb2);
                        },
                        function (_, cb2) {
                            zoneadm(['-u', payload.uuid, 'attach'], log, cb2);
                        }
                    ]}, function (e) {
                        cb(e);
                        return;
...
/*
 * Bhyve-specific reprovision function that replaces disk0 with a new image
 * while preserving all other disks and configuration.
 */
function reprovisionBhyve(uuid, vmobj, payload, log, cb)
{
    var disk0;
    var disk0_ds;
    var disk0_path;
    ...
    assert(vmobj.brand === 'bhyve', 'reprovisionBhyve called on non-bhyve VM');
    assert(payload.image_uuid, 'missing image_uuid in payload');

    log.info('Starting bhyve reprovision for VM %s', uuid);

    vasync.waterfall([
        function findDisk0(next) {
            // Find disk0 in the VM's disk list by looking for the one
            // ending with /disk0
            if (!vmobj.disks || vmobj.disks.length === 0) {
                next(new Error('No disks found for bhyve VM'));
                return;
            }

            var i;
            for (i = 0; i < vmobj.disks.length; i++) {
                if (vmobj.disks[i].path) {
                    // Extract dataset name from path
                    var ds = vmobj.disks[i].path
                        .replace(/^\/dev\/zvol\/rdsk\//, '')
                        .replace(/^\/dev\/zvol\/dsk\//, '');
                    // Check if this is disk0
                    if (ds.match(/\/disk0$/)) {
                        // Found disk0, verify it has boot property set
                        if (vmobj.disks[i].boot !== true) {
                            next(new Error('disk0 found but does not have '
                                + 'boot property set to true - reprovision '
                                + 'requires a bootable disk0'));
                            return;
                        }
                        disk0 = vmobj.disks[i];
                        disk0_ds = ds;
                        disk0_path = vmobj.disks[i].path;
                        break;
                    }
                }
            }
            ...
            if (!disk0) {
                next(new Error('disk0 not found for bhyve VM'));
                return;
            }

            log.info('Found disk0 at %s (dataset: %s)',
                disk0_path, disk0_ds);
            next();
        },
        function findSnapshot(next) {
            ...
            zfs(['get', '-Ho', 'value', 'type', snapname], log,
                function (err, fds) {
                    if (!err) {
                        log.debug('Using snapshot: %s', snapname);
                        snapshot = snapname;
                        next();
                        return;
                    }
                        ...
                        zfs(['get', '-Ho', 'value', 'type', snapname], log,
                            function (err2, fds2) {
                                if (!err2) {
                                    ...
                                    snapshot = snapname;
                                    next();
                                } else if (fds2.stderr.match(
                                    /dataset does not exist/)) {
                                    // Cannot create snapshot - image dataset
                                    // doesn't exist
                                    next(new Error('Image ' + payload.image_uuid
                                        + ' not found in zpool '
                                        + vmobj.zpool));
                                } else {
                                    next(err2);
        ...
        function destroyOldDisk0(next) {
            // Destroy the old disk0 - now that we've verified the
            // image snapshot exists
            log.info('Destroying old disk0: %s', disk0_ds);
            zfs(['destroy', '-r', '-f', disk0_ds], log, function (err, fds) {
                if (err) {
                    log.error({
                        err: err,
                        stdout: fds.stdout,
                        stderr: fds.stderr
                    }, 'Failed to destroy old disk0');
                    next(err);
                    return;
                }
                log.info('Successfully destroyed old disk0');
                next();
            });
        },
        function createNewDisk0(next) {
            var args = ['clone'];

            // Note: volblocksize is inherited from the snapshot and
            // cannot be changed
            // Note: volsize is also inherited from the snapshot

            // Clone from snapshot to new disk0
            args.push(snapshot);
            args.push(disk0_ds);

            log.info('Creating new disk0 from image %s', payload.image_uuid);
            zfs(args, log, function (err, fds) {
                if (err) {
                    log.error({
                        err: err,
                        stdout: fds.stdout,
                        stderr: fds.stderr
                    }, 'Failed to create new disk0');
                    next(err);
                    return;
                }
                log.info('Successfully created new disk0');
                next();
            });
        },
        function updateImageUuid(next) {
            // Update the image_uuid property on disk0 device in zone config
            // First get the current image-uuid value
            var info_cmd = 'info device match="' + disk0_path + '"';

            zonecfg(uuid, [info_cmd], {log: log}, function (err, fds) {
                if (err) {
                    log.error({
                        err: err,
                        stdout: fds.stdout,
                ...
                // Parse the output to find the current image-uuid value
                var lines = fds.stdout.split('\n');
                var current_uuid = null;
                // Match: property: (name=image-uuid, value="UUID")
                var uuid_regex = new RegExp(
                    'property:\\s*\\(name=image-uuid, value="([^"]+)"\\)'
                );

                for (var i = 0; i < lines.length; i++) {
                    var match = lines[i].match(uuid_regex);
                    if (match) {
                        current_uuid = match[1];
                        break;
                    }
                }
                ...
                var zcfg;
                if (!current_uuid) {
                    // No existing image-uuid, just add the new one
                    zcfg = 'select device match="' + disk0_path + '"; '
                        + 'add property (name=image-uuid, value="'
                        + payload.image_uuid + '"); '
                        + 'end';
                } else {
                    ...
                    zcfg = 'select device match="' + disk0_path + '"; '
                        + 'remove property (name=image-uuid, value="'
                        + current_uuid + '"); '
                        + 'add property (name=image-uuid, value="'
                        + payload.image_uuid + '"); '
                        + 'end';
                ...
                zonecfg(uuid, [zcfg], {log: log}, function (err2, fds2) {
                    if (err2) {
                        log.error({
                            err: err2,
                            stdout: fds2.stdout,
                            stderr: fds2.stderr
                        }, 'Failed to update image_uuid');
                        next(err2);
                        return;
                    }
                    log.info('Updated image_uuid from %s to %s',
                        current_uuid, payload.image_uuid);
                    next();
                });
            });
        },
        function runBrandInstall(next) {
            // Run the brand install script with -r flag for reprovision
            var args = ['-r', '-R', vmobj.zonepath, '-z', vmobj.zonename];
            var cmd = BRAND_OPTIONS[vmobj.brand].features
                .brand_install_script;

            log.info('Running brand install script for reprovision');
            traceExecFile(cmd, args, log, 'brand-install',
                function (error, stdout, stderr) {
                    if (error) {
                        log.error({err: error, stdout: stdout},
                            'Brand install script failed');
                        next(new Error('Error running brand install script'));
                        return;
                    }
                    log.debug('Brand install script output:\n%s', stderr);
                    next();
                });
        },
        function cleanupTransitionAndAutoboot(next) {
            // After brand install script, clean up transition and autoboot
            // The brand install script may have set a transition state
            VM.unsetTransition(vmobj, {log: log}, function (unset_err) {
                if (unset_err) {
                    log.error({err: unset_err},
                        'Failed to unset transition for bhyve VM');
                    // Continue anyway, don't fail the reprovision
                }

                // Check if we should autoboot
                if (vmobj.autoboot) {
                    log.info('Starting bhyve VM %s after reprovision', uuid);
                    VM.start(uuid, {}, {log: log}, function (start_err) {
                        if (start_err) {
                            log.error({err: start_err},
                                'Failed to start bhyve VM after reprovision');
                            // Don't fail the reprovision if start fails
                        }
                        next();
                    });
                } else {
        ...
        if (err) {
            log.error({err: err}, 'Bhyve reprovision failed');
        } else {
            log.info('Bhyve reprovision completed successfully');
        }
        cb(err);
    });
}

function validateBhyveDiskSizes(vmobj, payload, log, cb) {
    // Find disk0 in the VM's disk list
    var disk0;
    var disk0_ds;

    if (!vmobj.disks || vmobj.disks.length === 0) {
        cb(new Error('No disks found for bhyve VM'));
        return;
    }
    ...
    if (!disk0) {
        cb(new Error('disk0 not found for bhyve VM'));
        return;
    }

    // Check disk sizes
    var currentDiskSizeBytes;
    var newImageSizeMiB;

    vasync.waterfall([
        function getCurrentDiskSize(cb2) {
            // Get current disk0 volsize in bytes
            zfs(['get', '-Hp', 'volsize', disk0_ds], log,
                function (err, fds) {
                    if (err) {
                        log.error({
                            err: err,
                    ...
                    var fields = fds.stdout.trim().split('\t');
                    if (fields.length >= 3) {
                        currentDiskSizeBytes = parseInt(fields[2], 10);
                        log.debug('Current disk0 size: %d bytes',
                            currentDiskSizeBytes);
                        cb2();
                    } else {
                        cb2(new Error('Unexpected zfs volsize output'));
                    }
                });
        },
        function getNewImageSize(cb2) {
            // Get image manifest to check image_size
            var args = ['get', '-P', vmobj.zpool, payload.image_uuid];
            var cmd = '/usr/sbin/imgadm';
            ...
            traceExecFile(cmd, args, log, 'imgadm-get',
                function (error, stdout, stderr) {
                    ...
                    try {
                        ...
                        var manifest = data.manifest;
                        if (manifest && manifest.image_size) {
                            newImageSizeMiB = manifest.image_size;
                            log.debug('New image size: %d MiB',
                                newImageSizeMiB);
                            cb2();
                        } else {
                            cb2(new Error('Missing image_size in manifest'));
                        }
                    } catch (parseErr) {
                        cb2(new Error('Parse error: ' + parseErr.message));
                    }
                });
        },
        function compareSizes(cb2) {
            // Convert image size from MiB to bytes
            var newImageSizeBytes = newImageSizeMiB * 1024 * 1024;

            log.info('Size check: current=%d, new=%d bytes',
                currentDiskSizeBytes, newImageSizeBytes);

            if (newImageSizeBytes > currentDiskSizeBytes) {
                var currentMiB = Math.floor(
                    currentDiskSizeBytes / (1024 * 1024));
                var msg = 'Cannot reprovision: image (' + newImageSizeMiB
                    + ' MiB) > disk0 (' + currentMiB + ' MiB)';
                var err = new Error(msg);
                cb2(err);
            } else {
                log.info('Size check passed - proceeding');
                cb2();
            }
        }
    ], cb);
    ...
    vasync.waterfall([
        function (cb) {
            ...
            if (vmobj.hasOwnProperty('datasets') && vmobj.datasets.length > 1) {
                ...
                cb(new Error('cannot support reprovision with '
                    + 'non-standard "' + vmobj.datasets[0] + '" dataset'));
                return;
            }
            cb();
        }, function (cb) {
            // For bhyve, we validate zvol images for disk0 replacement
            // For other brands, we validate zone-dataset images for zoneroot
            if (vmobj.brand === 'bhyve') {
                // First validate the image, then check disk sizes
                vasync.waterfall([
                    function validateBhyveImage(next) {
                        validateImage({
                            types: ['zvol'],
                            uuid: payload.image_uuid,
                            zpool: vmobj.zpool
                    ...
                    function validateDiskSizes(next) {
                        validateBhyveDiskSizes(vmobj, payload, log, next);
                    },
                    function checkBhyveHolds(next) {
                        // Check for holds on disk0 before any state transitions
                        // Construct disk0 dataset name from zonepath
                        var disk0_ds = vmobj.zonepath.replace(/^\//, '')
                            + '/disk0';
                        log.info('Checking for holds on disk0 before '
                            + 'reprovision: %s', disk0_ds);
                        checkDiskHolds(disk0_ds, log, function (err) {
                            if (err) {
                                log.error({err: err},
                                    'Found holds on disk0 snapshots');
                                next(err);
                                return;
                            }
                            log.info('No holds found on disk0 snapshots');
                            next();
                        });
                    }
                ], cb);
                ...
                if (BRAND_OPTIONS[vmobj.brand].features.zoneroot_image_types) {
                    zoneroot_types = BRAND_OPTIONS[vmobj.brand]
                        .features.zoneroot_image_types;
                }
            ...
            VM.stop(uuid, {log: log}, function (e) {
                ...
                // wait for zone state installed here
                VM.load(uuid, {log: log}, function (l_err, l_obj) {
                    // If there was an error loading the vm at this point
                    // we don't know what's going on. This will fail the
                    // reprovision.
                    if (l_err) {
                        cb(l_err);
                        return;
                    }
                    VM.waitForZoneState(l_obj, 'installed',
                        {timeout: STOP_TIMEOUT},
                        function reprovisionStopWaitCb(err) {
                            if (err && err.hasOwnProperty('code')
                                && err.code === 'ETIMEOUT') {

                                log.info('Timeout waiting for shutdown to '
                                    + 'complete; halting.');
                                zoneadm(['-z', l_obj.zonename, 'halt'], log,
                        ...
                        // There was no error and the vm state should now be
                        // installed, or there was an error other than ETIMEOUT
                        // We'll float that error up. This will fail the
                        // reprovision.
                        cb(err);
                    });
                });
            ...
            // For bhyve, we handle reprovision differently -
            // we only replace disk0
            if (vmobj.brand === 'bhyve') {
                reprovisionBhyve(uuid, vmobj, payload, log, cb);
                return;
            }
            // Otherwise continue with native zone reprovision logic
            cb();
        }, function (cb) {
            // Skip native zone reprovision steps for bhyve
            if (vmobj.brand === 'bhyve') {
                cb();
                return;
            }
            ...
            var args;
            var retry_delay = 3; // second(s) between retries
            ...
            // Skip for bhyve - already done in reprovisionBhyve
            if (vmobj.brand === 'bhyve') {
                cb();
                return;
            }
            ...
            // Skip installZone for bhyve (already handled in reprovisionBhyve)
            if (vmobj.brand === 'bhyve') {
                cb();
                return;
            }

            // For non-bhyve brands, use installZone
            var p = {
                autoboot: true,
                reprovisioning: true,
                uuid: uuid,
