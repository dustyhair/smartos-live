...
var TEST_EXPERIMENTAL_UUID = '6bb89820-cd92-11e4-877f-63807ab2adaf';
...
test('setup8: rm experimental image ' + TEST_EXPERIMENTAL_UUID, function (t) {
    var cmd = format(
        'imgadm delete %s ||'
            + 'imgadm source -d https://updates.joyent.com ||'
            + 'imgadm source -d https://updates.joyent.com?channel=experimental'
            + ' || true',
...
// TEST_EXPERIMENTAL_UUID (and ORIGIN if necessary)
...
test('setup10: delete experimental image', function (t) {
    var cmd = format('imgadm delete %s || true', TEST_EXPERIMENTAL_UUID);
    exec(cmd, function () {
        t.end();
    });
});
